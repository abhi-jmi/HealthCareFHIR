...
public sealed class FhirBundleIngestionService(AppDbContext db, IFhirResourceClient fhirClient) : IFhirBundleIngestionService
{
    ...
    public async Task<FhirBundleIngestionResponse> IngestAsync(FhirBundleIngestionRequest request, string initiatedBy, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var bundle = _parser.Parse<Bundle>(request.BundleJson);
            ...
            var provenance = new Provenance { Recorded = Instant.Now(), Activity = new CodeableConcept("http://terminology.hl7.org/CodeSystem/v3-DataOperation", "CREATE", "create") };
        ...
        job.CompletedAt = DateTimeOffset.UtcNow; job.OperationOutcomeJson = _serializer.SerializeToString(outcome); await db.SaveChangesAsync(cancellationToken);
