...
public sealed class FhirLevelCatalogService : IFhirLevelCatalogService
{
    public FhirLevelCatalogDto GetCatalog() => new([
        ...
        new FhirLevelDto(4, "Record Keeping and Data Exchange", "FHIR resources for the healthcare process.", [
            ...
            Module("medications", "Medications", "Medication request, dispense, administration, statement and medication definitions.", ["Medication", "MedicationRequest", "MedicationDispense", "MedicationAdministration", "MedicationStatement"], ["/resources/MedicationRequest", "/resources/MedicationStatement", "/resources/MedicationAdministration"], ["/api/resources/{resourceType}"]),
            Module("workflow", "Workflow", "Task, Appointment, Schedule, referral/service request and planning resources.", ["Task", "Appointment", "Schedule", "Slot", "ServiceRequest", "Communication", "CommunicationRequest", "EpisodeOfCare"], ["/resources/Task", "/resources/Appointment", "/resources/Schedule", "/resources/ServiceRequest"], ["/api/resources/{resourceType}"]),
            Module("financial", "Financial", "Claim, Account, Invoice, ChargeItem, Coverage, EOB and payment resources.", ["Coverage", "Claim", "ClaimResponse", "ExplanationOfBenefit", "Invoice", "Account", "ChargeItem", "PaymentNotice", "PaymentReconciliation"], ["/resources/Coverage", "/resources/Claim", "/resources/ExplanationOfBenefit", "/resources/Invoice"], ["/api/resources/{resourceType}"])
        ]),
